        ...
        <div className="max-w-5xl mx-auto bg-white rounded-2xl shadow-xl p-8 mt-8">
            ...
            <div className="bg-white rounded-2xl shadow-xl p-6">
              <div className="flex justify-between items-center mb-4">
                <h2 className="text-xl font-bold text-blue-700">Dịch vụ</h2>
                <button
                              ...
                              <div className="flex gap-1">
                                {his.images.slice(0, 4).map((img, i) => (
                                  <img
                                    ...
                                    onClick={() => setImgModal({ open: true, images: his.images, idx: i })}
                                  />
                                ))}
                              </div>
              ...
              <div className="sm:hidden space-y-3">
                {histories.length === 0 ? (
                  <div className="text-center py-6 text-gray-400">Chưa có lịch sử sử dụng dịch vụ</div>
                ) : (
                  histories.map((his, idx) => (
                    <div key={(his._id || his.id || idx) + '-' + idx} className="bg-blue-50 rounded-xl shadow p-4 flex flex-col gap-2 relative">
                      ...
                      <div className="text-sm text-gray-700"><b>Nhân viên:</b> {staff.find(s => String(s._id || s.id) === String(his.staffId || his.staff_id))?.name || ''}</div>
                      <div className="text-sm text-gray-700"><b>Dịch vụ:</b> {services.find(s => String(s._id || s.id) === String(his.serviceId || his.service_id))?.name || ''}</div>
                      ...
                      {his.payment && <div className="text-sm text-gray-700"><b>Thanh toán:</b> {his.payment || his.payment_method || ''}</div>}
